...
class TestConsole(unittest.TestCase):
    ...
    def test_create_v2_params(self):
        ...
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd('create User email="abebe12@gmail.com"\
                               password="abebeboss"\
                               first_name="Abebe" last_name')
            id = f.getvalue()[:-1]
            self.assertEqual(len(id), 36)
        with patch('sys.stdout', new=StringIO()) as f:
            self.consol.onecmd("show User {}".format(id))
            print(f.getvalue())
            self.assertTrue("'email': 'abebe12@gmail.com'" in f.getvalue())
            self.assertTrue("'password': 'abebeboss'" in f.getvalue())
            self.assertTrue("'first_name': 'Abebe'" in f.getvalue())
